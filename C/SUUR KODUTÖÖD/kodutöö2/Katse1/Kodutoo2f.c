...
int main(void)
{
	int massiivideSuurus = MassiiviSuuruseLeidmine();
	...
	MassiivideValjaPrintimine(massiivideSuurus, kgHind, hind, kaal);
	printf("\n\n");
...
void MassiivideSisestus(int mSuurus, float euro[], float kg[])
{
	...
	while (i < mSuurus)
    {
		printf("Sisestage hind (%d/%d): ", i + 1, mSuurus);
		scanf("%f", &euro[i]);
		i++;
	}
	printf("\n");
...
void MassiivideValjaPrintimine(int mSuurus, float kiloHind[], float euro[], float kg[])
{
	...
	for (i = 0; i < mSuurus; i++)
	{
		...
		printf("%3.f %3.f %6.2f", euro[i], kg[i], kiloHind[i]);
		printf("\n");
	}
}
...
//~ void MassiiviKaalSisestus(int mSuurus, float kg[])
//~ {
	//~ int i = 0;
	//~ while (i < mSuurus)
    //~ {
